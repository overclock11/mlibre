        ...
        <div className="item-detail__item-space">
            <div className="item-detail">
                <Grid container justify="space-around" className="item-detail__product-detail">
                    ...
                    <Grid item sm={4}>
                        <p>{itemDetail?.condition.toLowerCase() === 'new' ? "Nuevo " : "Usado "} - {itemDetail?.sold_quantity} vendidos</p>
                        <h2 className="item-detail__title">{itemDetail?.title}</h2>
                        <span className="item-detail__title">$ {itemDetail?.price.amount.toLocaleString(navigator.language, { minimumFractionDigits: 2,currency: itemDetail?.price.currency, style:'decimal'})}</span>
                ...
                <Grid container>
                    {itemDetail?.description !== '' ? (<Grid item sm={7}>
